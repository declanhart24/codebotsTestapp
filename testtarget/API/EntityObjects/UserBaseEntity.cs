	...
	public abstract class UserBaseEntity : BaseEntity
	{
		...
		public Guid CreateUser(bool isRegistered = true)
		{
			...
			if (isRegistered)
			{
				...
				// % protected region % [Adjust the db context] off begin
				using (var context = new TestDBContext(configure.DbContextOptions, null, null))
				{
					context.Users.FirstOrDefault(x => x.UserName == EmailAddress).EmailConfirmed = true;
					context.SaveChanges();
				}
				// % protected region % [Adjust the db context] end
			}
			return Id;
		}
	}
