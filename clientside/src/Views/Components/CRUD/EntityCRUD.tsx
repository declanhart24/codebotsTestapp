...
class EntityCRUD<T extends Model> extends React.Component<IEntityCRUDProps<T>> {
	...
	private url = () => {
		if (this.props.URLExtension) {
			return `${this.props.match.url}/${this.props.URLExtension}`
		}
		...
		return (
			<div>
				<Switch>
					...
					<Route path={`${this.url()}/view/:id`} render={entityViewPage} />
					<Route exact={true} path={`${this.url()}/create`} render={entityCreatePage} />
					<Route path={`${this.url()}/edit/:id`} render={entityEditPage} />
				</Switch>
			</div>
		);
	}
