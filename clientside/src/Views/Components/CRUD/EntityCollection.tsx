...
class EntityCollection<T extends Model> extends React.Component<IEntityCollectionProps<T>, any> {
	...
	protected renderCreateButton(): React.ReactNode {
		...
		return (
			<Button
				...
				buttonProps={{ onClick: () => { this.props.history.push(`${this.props.url}/create`); } }}>
	...
	protected getTableActions = (refetch: () => void) => {
		...
		if(SecurityService.canRead(this.props.modelType)){
			tableActions.push({
				action: (item) => {
					this.props.history.push({ pathname: `${this.props.url}/view/${item['id']}` });
				},
				label: "View",
				showIcon: true,
				icon: "look",
		...
		if(SecurityService.canUpdate(this.props.modelType)){
			tableActions.push({
				action: (item) => {
					this.props.history.push({ pathname: `${this.props.url}/edit/${item['id']}` });
				},
				label: "Edit",
				showIcon: true,
				icon: "edit",
