...
using Test.Services.Scheduling;
using Test.Services.Scheduling.Tasks;
using Test.Services.CertificateProvider;
	...
	public class Startup
	{
		...
		public IServiceProvider ConfigureServices(IServiceCollection services)
		{
			services
				.AddMvc(options =>
				{
					// % protected region % [Configure MVC options here here] off begin
					options.Filters.Add(new XsrfActionFilterAttribute());
					options.Filters.Add(new AntiforgeryFilterAttribute());
					// % protected region % [Configure MVC options here here] end
				})
			...
			{
				options.SwaggerDoc("json", new OpenApiInfo {Title = "Test", Version = "v1"});
			...
			services.AddSingleton<InvoiceEntityType>();
			services.AddSingleton<InvoiceEntityInputType>();


			// Connect the database type to the GraphQL type
			GraphTypeTypeRegistry.Register<InvoiceEntity, InvoiceEntityType>();
			...
			EfGraphQLConventions.RegisterInContainer<TestDBContext>(services);
			...
			{
				options.InvalidModelStateResponseFactory = ctx => new TestActionResult();
			});
			...
			DataSeedHelper dataSeed)
		{
			Audit.Core.Configuration.Setup()
				.UseEntityFramework(ef => ef
					...
					{
						var context = entry.GetEntry().Context as TestDBContext;
			...
			{
				options.SwaggerEndpoint("/api/swagger/json/openapi.json", "Test");
